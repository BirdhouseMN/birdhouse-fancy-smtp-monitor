...
function bfsmtp_status_check($request) {
    ...
    $message = 'This is a test email sent by the Birdhouse SMTP Monitor plugin.';
    ...
    if ($mode === 'auto') {
        $to = 'noreply@birdhousemanager.com';
        ob_start();
        $sent = wp_mail($to, $subject, $message, $headers);
        $debug_output = ob_get_clean();

        $transport = $phpmailer->Mailer ?? 'unknown';
        $error_info = $phpmailer->ErrorInfo ?? 'Unavailable';

        $status = ($sent && empty($error_info)) ? 'ok' : 'fail';
        $http   = ($status === 'ok') ? 200 : 500;
